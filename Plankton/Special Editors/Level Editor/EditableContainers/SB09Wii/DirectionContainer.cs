...
{
    public class DirectionContainer : EditableContainer
    {
        public Direction asset;

        public DirectionContainer(TOCEntry entry)
        {
            this.entry = entry;
            isInstanced = false;

            asset = (Direction)entry.entity;
        }

        public override Vector3 GetPosition() // Used for focusing on object
        {
            return asset.location.GetVector3();
        }
        public override void SetPosition(Vector3 pos)
        {
            asset.location = new HoArchive.float3(pos);
        }

        public override Vector3 GetRotation()
        {
            return asset.direction.GetVector3();
        }

        public override void SetRotation(Vector3 rot)
        {
            asset.direction = new float3(rot);
        ...
        public override Matrix4x4 GetInstanceMatrix()
        {
            ...
            Vector3 ypr = asset.direction.GetVector3();
        ...
        public override void AddRenderInstances(PrimitiveInstance baseinstance, RenderHelper helper)
        {
            ...
            baseinstance.matrix = Matrix4x4.CreateFromYawPitchRoll((float)(-Math.PI / 2.0), 0, 0) * GetInstanceMatrix();
            baseinstance.color = GlobalRenderSettings.directionColor;

            helper.AddLine(baseinstance);
        }
